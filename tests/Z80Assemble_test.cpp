...
TEST_CASE(ExpressionOptions) {
    Z80Assembler<Z80DefaultBus>::Options options;

    // 1. Test expressions.enabled = false
    options = Z80Assembler<Z80DefaultBus>::get_default_options();
    options.expressions.enabled = false;
    ASSERT_CODE_WITH_OPTS("LD A, 10", {0x3E, 10}, options); // Numeric literal should work
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, 10 + 5", options); // Arithmetic expression should fail
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, HIGH(0x1234)", options); // Function should fail
    ASSERT_COMPILE_FAILS_WITH_OPTS("VAL EQU 10 + 5\nLD A, VAL", options); // EQU with expression should fail

    // 2. Test expressions.allow_functions = false
    options = Z80Assembler<Z80DefaultBus>::get_default_options();
    options.expressions.allow_functions = false;
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, HIGH(0x1234)", options);
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, LOW(0xABCD)", options);
    ASSERT_CODE_WITH_OPTS("LD A, 10 + 5", {0x3E, 15}, options); // Other expressions should still work

    // 3. Test expressions.allow_arithmetic = false
    options = Z80Assembler<Z80DefaultBus>::get_default_options();
    options.expressions.allow_arithmetic = false;
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, 10 + 5", options);
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, 10 - 5", options);
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, 10 * 5", options);
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, 10 / 5", options);
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, 10 % 5", options);
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, -5", options); // Unary minus
    ASSERT_CODE_WITH_OPTS("LD A, HIGH(0x1234)", {0x3E, 0x12}, options); // Other expressions should still work
    ASSERT_CODE_WITH_OPTS("LD A, 0b1100 & 0b0101", {0x3E, 0b0100}, options); // Bitwise should still work

    // 4. Test expressions.allow_bitwise = false
    options = Z80Assembler<Z80DefaultBus>::get_default_options();
    options.expressions.allow_bitwise = false;
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, 0b1100 & 0b0101", options);
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, 0b1100 | 0b0101", options);
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, 0b1100 ^ 0b0101", options);
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, 5 << 2", options);
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, 20 >> 1", options);
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, ~0", options); // Bitwise NOT
    ASSERT_CODE_WITH_OPTS("LD A, 10 + 5", {0x3E, 15}, options); // Arithmetic should still work

    // 5. Test expressions.allow_logical = false
    options = Z80Assembler<Z80DefaultBus>::get_default_options();
    options.expressions.allow_logical = false;
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, (1 && 0)", options);
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, (1 || 0)", options);
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, !1", options); // Logical NOT
    ASSERT_CODE_WITH_OPTS("LD A, 10 + 5", {0x3E, 15}, options); // Arithmetic should still work

    // 6. Test expressions.allow_comparison = false
    options = Z80Assembler<Z80DefaultBus>::get_default_options();
    options.expressions.allow_comparison = false;
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, 10 > 5", options);
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, 10 < 5", options);
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, 10 == 5", options);
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, 10 != 5", options);
    ...
    // 7. Test combinations (e.g., only arithmetic allowed)
    options = Z80Assembler<Z80DefaultBus>::get_default_options();
    options.expressions.allow_functions = false;
    options.expressions.allow_bitwise = false;
    options.expressions.allow_logical = false;
    options.expressions.allow_comparison = false;
    ASSERT_CODE_WITH_OPTS("LD A, 10 + 5 * 2", {0x3E, 20}, options); // Arithmetic works
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, HIGH(0x1234)", options); // Functions fail
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, 10 & 5", options); // Bitwise fail

    // 8. Test that disabling expressions.enabled overrides individual allows
    options = Z80Assembler<Z80DefaultBus>::get_default_options();
    options.expressions.enabled = false;
    options.expressions.allow_arithmetic = true; // This should be ignored
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, 10 + 5", options);

    // 9. Test complex expression with some parts disabled
    options = Z80Assembler<Z80DefaultBus>::get_default_options();
    options.expressions.allow_bitwise = false;
    ASSERT_COMPILE_FAILS_WITH_OPTS("VAL EQU (10 + 5) & 0xFF\nLD A, VAL", options); // Bitwise part should fail
    options = Z80Assembler<Z80DefaultBus>::get_default_options();
    options.expressions.allow_arithmetic = false;
    ASSERT_COMPILE_FAILS_WITH_OPTS("VAL EQU HIGH(10 + 5)\nLD A, VAL", options); // Arithmetic part should fail

    // 10. Test default options (all enabled)
    options = Z80Assembler<Z80DefaultBus>::get_default_options();
    ASSERT_CODE_WITH_OPTS("LD A, 10 + 5", {0x3E, 15}, options);
    ASSERT_CODE_WITH_OPTS("LD A, HIGH(0x1234)", {0x3E, 0x12}, options);
    ASSERT_CODE_WITH_OPTS("LD A, 10 & 5", {0x3E, 0x00}, options);
    ASSERT_CODE_WITH_OPTS("LD A, !0", {0x3E, 0x01}, options);
    ASSERT_CODE_WITH_OPTS("LD A, 10 == 10", {0x3E, 0x01}, options);

    // 11. Test with a symbol that needs evaluation
    options = Z80Assembler<Z80DefaultBus>::get_default_options();
    options.expressions.allow_arithmetic = false;
    ASSERT_COMPILE_FAILS_WITH_OPTS("MY_VAL EQU 5\nLD A, MY_VAL + 2", options);
    options = Z80Assembler<Z80DefaultBus>::get_default_options();
    options.expressions.allow_arithmetic = true;
    ASSERT_CODE_WITH_OPTS("MY_VAL EQU 5\nLD A, MY_VAL + 2", {0x3E, 7}, options);

    // 12. Test with complex expression and specific options disabled
    options = Z80Assembler<Z80DefaultBus>::get_default_options();
    options.expressions.allow_bitwise = false;
    ASSERT_COMPILE_FAILS_WITH_OPTS(R"(
        ...
        VAL2 EQU 10
        VAL3 EQU 0x40
        COMPLEX_VAL EQU (((VAL1 << 2) + (VAL2 * 3)) & 0x7F) | (VAL3 - (20 / 2))
        LD A, COMPLEX_VAL
    )", options);
    ...
    options = Z80Assembler<Z80DefaultBus>::get_default_options();
    options.expressions.allow_logical = false;
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, (10 > 5) && (20 < 30)", options); // Should fail as logical is not allowed
    options = Z80Assembler<Z80DefaultBus>::get_default_options();
    options.expressions.allow_comparison = false;
    ASSERT_COMPILE_FAILS_WITH_OPTS("LD A, (10 > 5) && (20 < 30)", options); // Should fail as comparison is not allowed, which is part of the logical expression
}

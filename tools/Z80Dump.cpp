...
bool load_bin_file(Z80DefaultBus &bus, const std::vector<uint8_t> &data,
                   uint16_t load_addr) {
    for (size_t i = 0; i < data.size(); ++i) {
        if (load_addr + i > 0xFFFF) {
            std::cerr << "Warning: Binary file too large, truncated at 0xFFFF."
                      << std::endl;
            break;
        }
...
bool load_sna_file(Z80<> &cpu, const std::vector<uint8_t> &data) {
    ...
    state.m_PC.w = cpu.get_bus()->peek(state.m_SP.w) |
                   (cpu.get_bus()->peek(state.m_SP.w + 1) << 8);
    state.m_SP.w += 2;
...
bool load_z80_file(Z80<> &cpu, const std::vector<uint8_t> &data) {
    ...
    cpu.restore_state(state);
    if (state.m_PC.w == 0) {
        std::cerr << "Error: Z80 v2/v3 files are not supported yet."
                  << std::endl;
        return false;
    }
    size_t data_ptr = 30;
    if (compressed) {
        ...
        while (data_ptr < data.size() && mem_addr < 0xFFFF) {
            if (data_ptr + 1 < data.size() && data[data_ptr] == 0xED &&
                data[data_ptr + 1] == 0xED) {
        ...
        if (data.size() - 30 != 49152) {
            std::cerr << "Error: Invalid uncompressed 48K Z80 file size."
                      << std::endl;
            return false;
        }
        for (int i = 0; i < 3; ++i) {
...
uint16_t resolve_address(const std::string &addr_str, const Z80<> &cpu) {
    ...
    size_t operator_pos =
        (plus_pos != std::string::npos) ? plus_pos : minus_pos;
    ...
    if (operator_pos != std::string::npos) {
        ...
        try {
            ...
            throw std::runtime_error("Invalid offset in address expression: " +
                                     offset_str);
        }
    ...
    try {
        ...
        if (upper_str.size() > 2 && upper_str.substr(0, 2) == "0X") {
            ...
            return std::stoul(upper_str.substr(0, upper_str.length() - 1),
                              nullptr, 16);
        } else {
        ...
        if (upper_str == "IY")
            return cpu.get_IY();
        throw std::runtime_error("Invalid address or register name: " +
                                 addr_str);
...
int main(int argc, char *argv[]) {
    ...
    std::cout << "Loading file: " << file_path
              << " (type: " << (ext.empty() ? "bin" : ext) << ")" << std::endl;
    if (ext == "sna") {
    ...
    if (!loaded) {
        std::cerr << "Error: Failed to load file content into emulator."
                  << std::endl;
        return 1;
    }
    std::cout << "File loaded successfully.\n" << std::endl;
    if (run_ticks > 0) {
        std::cout << "--- Running emulation for " << run_ticks
                  << " T-states ---\n";
        long long executed_ticks = cpu.run(cpu.get_ticks() + run_ticks);
        std::cout << "Executed " << executed_ticks
                  << " T-states. CPU is now at tick " << cpu.get_ticks()
                  << ".\n"
                  << std::endl;
    }
    if (run_steps > 0) {
        std::cout << "--- Running emulation for " << run_steps
                  << " instructions (steps) ---\n";
        long long total_ticks_for_steps = 0;
        for (long long i = 0; i < run_steps; ++i) {
            total_ticks_for_steps += cpu.step();
        }
        std::cout << "Executed " << run_steps << " instructions ("
                  << total_ticks_for_steps << " T-states). CPU is now at tick "
                  << cpu.get_ticks() << ".\n"
                  << std::endl;
    }
    ...
    if (reg_dump_action) {
        std::string format = reg_dump_format.empty()

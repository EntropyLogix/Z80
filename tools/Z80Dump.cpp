...
void print_usage() {
    std::cerr << "Usage: Z80Dump <file_path> [options]\n"
            ...
            << "Options:\n"
            << "  --mem-dump <address_hex> <bytes_hex>\n"
            << "    Dumps memory starting from <address_hex> for <bytes_hex> bytes.\n"
            << "    Example: --mem-dump 4000 100\n\n"
            << "  --disassemble <address_hex> <lines_dec>\n"
            << "    Disassembles <lines_dec> lines of code starting from <address_hex>.\n"
...
bool load_z80_file(Z80<>& cpu, const std::vector<uint8_t>& data) {
    ...
    if (byte12 == 0xFF) byte12 = 1; // Handle v1 quirk
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 2; i < argc; ++i) {
        ...
        if (arg == "--mem-dump" && i + 2 < argc) {
            mem_dump_addr = std::stoul(argv[++i], nullptr, 16);
            mem_dump_size = std::stoul(argv[++i], nullptr, 16);
        } else if (arg == "--disassemble" && i + 2 < argc) {
            disasm_addr = std::stoul(argv[++i], nullptr, 16);

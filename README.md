...
Additionally there is set of **command-line tools** built on top of it.
...
## Analysis and Assembly Classes

### Z80Analyze Class (`Z80Analyze.h`)
The `Z80Analyze` class offers powerful capabilities for inspecting and understanding Z80 machine code and CPU state. It provides:
*   **Memory Dumping:** Formats and displays memory regions with options for hexadecimal, decimal, and ASCII representations.
*   **Register Dumping:** Presents the current state of all Z80 registers (main, alternate, index, special) in various formats (hex, decimal, flags string).
*   **Disassembly:** Converts machine code into human-readable Z80 assembly mnemonics. It supports various formatting options for addresses, bytes, mnemonics, and T-states.
*   **Label Support:** Can load labels from `.map` (e.g., generated by assemblers) and `.ctl` (Spectaculator format) files to enhance disassembly output with meaningful symbol names instead of raw addresses.

### Z80Assembler Class (`Z80Assemble.h`)
The `Z80Assembler` class is a two-pass assembler designed to convert Z80 assembly source code into executable machine code. Key features include:
*   **Two-Pass Assembly:** Resolves forward references and symbols correctly.
*   **Symbol Management:** Tracks and resolves labels and constants (`EQU`).
*   **Code Generation:** Emits machine code bytes into the provided memory bus.
*   **Directives Support:** Handles common directives like `ORG`, `DB`/`DEFB`, `DW`/`DEFW`, `DS`/`DEFS`.
*   **Expression Evaluation:** Supports basic arithmetic and logical expressions in operands.
*   **Error Handling:** Provides detailed error messages for syntax issues, undefined symbols, and out-of-range jumps.
